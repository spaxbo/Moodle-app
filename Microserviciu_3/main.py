...
from schemas import LoginRequestModel, ValidateTokenRequestModel, LogoutRequestModel, CreateUserRequestModel, ChangePasswordRequestModel, DeleteUserRequestModel
...
@app.post("/createUser")
async def create_user(request: CreateUserRequestModel, user_info: dict = Depends(validate_bearer_token)):
    ...
    grpc_request_create = idm_service_pb2.CreateUserRequest(
        username = request.username,
        password = request.password,
        role = request.role
    )
    ...
    try:
        grpc_response_create = stub.CreateUser(grpc_request_create)
        if not grpc_response_create.success:
            raise HTTPException(status_code=403, detail=grpc_response_create.message)

        return {"message": grpc_response_create.message}
